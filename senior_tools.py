...
from PyQt6.QtGui import QKeySequence, QPainter, QPainterPath, QRegion, QShortcut, QAction
...
EMBEDDING_CACHE_MAX_ENTRIES = max(1, int(os.getenv("EMBEDDING_CACHE_MAX_ENTRIES", "10000")))
EMBEDDING_CACHE_ENABLED = os.getenv("EMBEDDING_CACHE_ENABLED", "true").lower() == "true"
EMBEDDING_CACHE_TTL_DAYS = max(1, int(os.getenv("EMBEDDING_CACHE_TTL_DAYS", "30")))  # Cache entries expire after N days
        ...
        try:
            ...
            if cache_file.exists():
                try:
                    with open(cache_file, 'r', encoding='utf-8') as f:
                        ...
                        if "_stats" in cache_data and _cache_stats["hits"] == 0 and _cache_stats["misses"] == 0:
            ...
            if cache_key in cache_data:
                ...
                if _is_cache_entry_expired(cache_entry, EMBEDDING_CACHE_TTL_DAYS):
                    logger.info(f"Cache entry expired (key: {cache_key[:8]}..., age: {EMBEDDING_CACHE_TTL_DAYS}+ days)")
                    ...
                    if 'embedding_compressed' in cache_entry:
                        embedding = _decompress_embedding(cache_entry['embedding_compressed'])
                    elif 'embedding' in cache_entry:
                        embedding = cache_entry['embedding']
                    else:
                        # Corrupted entry, treat as miss
                        logger.warning(f"Cache entry missing embedding data (key: {cache_key[:8]}...)")
                        del cache_data[cache_key]
                        with _cache_stats_lock:
                    ...
                    logger.info(f"Embedding cache HIT (key: {cache_key[:8]}..., hits: {cache_entry['hits']}, hit_rate: {hit_rate:.1f}%)")
                    return embedding

            # Cache miss - generate embedding
            with _cache_stats_lock:
                _cache_stats["misses"] += 1
            logger.info(f"Embedding cache MISS (key: {cache_key[:8]}...) - generating new embedding")
            embedding = await get_embedding(text)

            # Clean expired entries before checking size limit
            expired_keys = [
                key for key, entry in cache_data.items()
                if key != "_stats" and _is_cache_entry_expired(entry, EMBEDDING_CACHE_TTL_DAYS)
            ]
            ...
            if cache_entries_count >= EMBEDDING_CACHE_MAX_ENTRIES:
                logger.info(f"Cache size limit reached ({cache_entries_count}/{EMBEDDING_CACHE_MAX_ENTRIES}), performing LRU eviction")
                ...
                num_to_remove = max(1, int(EMBEDDING_CACHE_MAX_ENTRIES * eviction_percentage))
                for old_key, _ in sorted_entries[:num_to_remove]:
                    del cache_data[old_key]
                    with _cache_stats_lock:
                        _cache_stats["evictions"] += 1

                logger.info(f"Evicted {num_to_remove} old cache entries, new size: {len(cache_data) - 1}")
            ...
            try:
                with _cache_stats_lock:
                    _cache_stats["total_size_bytes"] = cache_file.stat().st_size if cache_file.exists() else 0
            ...
            with _cache_stats_lock:
                hit_rate = _cache_stats["hits"] / (_cache_stats["hits"] + _cache_stats["misses"]) * 100 if (_cache_stats["hits"] + _cache_stats["misses"]) > 0 else 0
            logger.info(f"Saved embedding to cache (total entries: {len(cache_data) - 1}, hit_rate: {hit_rate:.1f}%)")
            return embedding

        except Exception as e:
            logger.error(f"Cache operation failed, falling back to direct API call: {e}")
...
def get_cache_statistics_internal() -> dict:
    ...
    try:
        ...
        with _cache_stats_lock:
            ...
            cost_per_embedding = openai_cost_per_embedding if EMBEDDING_PROVIDER == "openai" else 0.0
...
def setup_logging():
    ...
    if not senior_tools_handler_exists:
        ...
        file_handler = FlushingFileHandler(log_file, mode='a', encoding='utf-8', delay=False)
        ...
        has_stream_handler = any(isinstance(h, logging.StreamHandler) and not hasattr(h, 'baseFilename')
...
def setup_cursor_config_files(target_dir: str = None):
    ...
    try:
        ...
        try:
            ...
            logger.info("✅ Successfully loaded rules content from config/agent_rules.md")
        ...
        if not command_file.exists():
            ...
            if existing_content != command_content:
                ...
                logger.info(f"Command file already exists with correct content: {command_file}")
        ...
        if not rules_file.exists():
            ...
            if existing_content != rules_content.strip():
                ...
                logger.info(f"Rules file already exists with correct content: {rules_file}")

        # Summary message
        if command_updated or rules_updated:
            logger.info("✅ Cursor configuration files have been set up/updated successfully!")
        else:
            logger.info("✅ Cursor configuration files already exist with correct content")
...
def _pregrab_cursor_windows_at_startup():
    ...
    disable_cursor = os.getenv("APP_DISABLE_CURSOR_CONTROL", "").strip().lower() in ("1", "true", "yes", "on")
    if disable_cursor:
        ...
        try:
            ...
            if platform.system() == "Windows":
                try:
                    ...
                    logger.debug("Worker thread: COM initialized as MULTITHREADED for pywinauto")
                except Exception as e:
            ...
            if _cached_cursor_windows:
                logger.info(f"✓ Pre-grabbed {len(_cached_cursor_windows)} Cursor window(s) at MCP startup (prioritized by CWD match)")
            else:
    ...
    if thread.is_alive():
        logger.warning("Pregrab thread still running after 5s timeout - continuing anyway")
...
class ModernTaskMasterDialog(QDialog):
        ...
        WIDGET_BORDER_RADIUS = 8   # Widget rounded corners (matches HTML BORDER_RADIUS_XLARGE)

        def __init__(self, agent_comment: str = "", dark_mode: bool = True, project_dir: str = None, parent=None):
            ...
            self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)  # Enable transparency for rounded corners (Qt6)
            self.setModal(True)
        ...
        def _load_header_image(self):
            ...
            try:
                ...
                if header_path.exists():
                    ...
                    target_height = self.header_title.height() or self.header_title.sizeHint().height() or 32
        ...
        def refresh_theme(self):
            ...
            try:
                ...
                dark_pref = os.getenv("APP_DARK_MODE", "true").strip().lower() in ("1", "true", "yes", "on")
                ...
                logger.info(f"Theme switching from {'dark' if old_dark_mode else 'light'} to {'dark' if dark_pref else 'light'}")
                ...
                if app is not None:
                    ...
                    qss_paths = [
                        script_dir / ("styles/app_dark.qss" if preferred_dark else "styles/app_light.qss"),
                        script_dir / ("styles/app_light.qss" if preferred_dark else "styles/app_dark.qss")
                    ]
                    qss_loaded = False
                    for p in qss_paths:
                        try:
        ...
        def _update_sparkle_icon(self):
            ...
            try:
                ...
                icon_file = "sparkle_icon_dark.png" if self.dark_mode else "sparkle_icon.png"
        ...
        def _apply_rounded_corners_to_widget(self, widget: QWidget, radius: int = None) -> None:
            """
            ...
            try:
                ...
                if hasattr(widget, '_rounded_corners_applied'):
                    logger.debug(f"Rounded corners already applied to {widget.__class__.__name__}")
                    return
                ...
                def update_widget_mask() -> None:
                    ...
                    # Fix #3: Specific exception handling with appropriate logging
                        ...
                        logger.debug(f"Failed to update widget mask for {widget.__class__.__name__}: {e}")
                    except Exception as e:
                        # Unexpected errors should be logged at warning level
                        logger.warning(f"Unexpected error updating widget mask for {widget.__class__.__name__}: {e}", exc_info=True)
                ...
                def cleanup():
                    try:
                        ...
                        logger.debug(f"Cleaned up rounded corners for {widget.__class__.__name__}")
                    except Exception:
                        pass
            ...
            # Fix #3: Specific exception handling for outer try block
                ...
                logger.debug(f"Failed to apply rounded corners to {widget.__class__.__name__}: {e}")
            except Exception as e:
                # Unexpected errors
                logger.warning(f"Unexpected error applying rounded corners to {widget.__class__.__name__}: {e}", exc_info=True)
        ...
        def create_agent_section(self, layout):
            ...
            try:
                ...
                logger.info(f"Display project name: {project_name} (from path: {project_path})")
            ...
            try:
                ...
                logger.debug(f"Markdown rendered successfully, HTML length: {len(html_content)}")
...
'''
            ...
            self.brain_button = QToolButton(agent_frame)  # Parent to frame instead of web view
            ...
            try:
                ...
                self.brain_button.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground, True)
                self.brain_button.setAttribute(Qt.WidgetAttribute.WA_NoSystemBackground, True)
            except Exception:
                pass  # Qt6
        ...
        def create_response_section(self, layout):
            ...
            try:
                ...
                self.logger.error(
                    f"Error initializing slash command support: {e}",
                    exc_info=True
                )
                self.response_text = QTextEdit()

            self.response_text.setAccessibleName("Response Input")
            self.response_text.setPlaceholderText("Enter your feedback here (type '/' for commands)...")
            ...
            self.response_text.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
            ...
            try:
                ...
                self.sparkle_button.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground, True)
                self.sparkle_button.setAttribute(Qt.WidgetAttribute.WA_NoSystemBackground, True)
            except Exception:
                pass  # Qt6
            ...
            response_frame.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        ...
        def _is_cursor_control_disabled(self):
            ...
            return os.getenv("APP_DISABLE_CURSOR_CONTROL", "").strip().lower() in ("1", "true", "yes", "on")

        def _is_auto_stop_enabled(self):
            """Check if auto-stop functionality is enabled"""
            return os.getenv("APP_AUTO_STOP", "").strip().lower() in ("1", "true", "yes", "on")
        ...
        def copy_agent_text(self):
            ...
            try:
                ...
                logger.info(f"Focused widget: {focused_widget} (type: {type(focused_widget).__name__})")
                ...
                if hasattr(self, 'agent_text') and self.agent_text.hasFocus():
                    ...
                    logger.info(f"agent_text doesn't have focus, delegating to focused widget")
        ...
        def eventFilter(self, obj, event):
            ...
            if hasattr(self, 'response_text') and obj == self.response_text:
                ...
                try:
                    if hasattr(self, "_position_sparkle_on_response_text") and event.type() in (QEvent.Type.Resize, QEvent.Type.Show, QEvent.LayoutRequest):
            ...
            if hasattr(self, 'agent_text') and obj == self.agent_text:
                try:
                    if hasattr(self, "_position_brain_on_agent_text") and event.type() in (QEvent.Type.Resize, QEvent.Type.Show, QEvent.LayoutRequest):
            ...
            try:
                ...
                if mimeData.hasImage():
                    ...
                    if image_manager.add_image_from_clipboard():
                        ...
                        logger.warning("Image detected in clipboard but failed to add - allowing text paste")
        ...
        def remove_individual_image(self, index):
            ...
            try:
                ...
                total_images = len(image_manager.image_descriptions) + len(image_manager.pending_images)
                if total_images == 0:
                    self.remove_attachment_text_from_input()
                    # Keep response editor height unchanged when images are cleared
                    try:
        ...
        def remove_attachment_text_from_input(self):
            ...
            try:
                ...
                current_text = re.sub(r'\n\s*\n\s*\n', '\n\n', current_text)  # Multiple newlines to double
        ...
        def update_images_display(self):
            ...
            for i, _ in enumerate(image_manager.pending_images):
                self.add_image_item_to_grid(len(image_manager.image_descriptions) + i, False, current_row, current_col)
                current_col += 1
                if current_col >= images_per_row:
                    current_col = 0
                    current_row += 1
        ...
        def adjust_window_height_for_images(self):
            ...
            try:
                ...
                self.resize_animation.setEndValue(QRect(self.x(), self.y(), current_width, target_height))
        ...
        def show_memory_dialog(self):
            ...
            try:
                if not self.project_dir:
                    QMessageBox.information(self, "No Project", "No project directory available.")
                    return
                ...
                if "error" in memory_data:
                    QMessageBox.information(self, "No Memory", f"No memory data available: {memory_data.get('error', 'Unknown error')}")
                    return
                ...
                if any(counts.values()):
                    ...
                    if counts.get('user_preference', 0) > 0:
                        memory_content += f"• User Preference: {counts['user_preference']}\n"
                    memory_content += "\n"
        ...
        def improve_with_ai(self):
            ...
            try:
                ...
                QMessageBox.warning(self, "Error", f"Google GenAI library not available: {e}")
                return
...
Return only the improved text, no explanations."""
                all_images = []
                if hasattr(image_manager, 'processed_images') and image_manager.processed_images:
                    all_images.extend(image_manager.processed_images)
                if hasattr(image_manager, 'pending_images') and image_manager.pending_images:
                    all_images.extend(image_manager.pending_images)
                if hasattr(image_manager, 'image_descriptions') and image_manager.image_descriptions:
            ...
            def poll():
                if result["done"]:
                    progress.reset()
                    if cancelled["v"]:
                        QMessageBox.information(self, "Cancelled", "AI improvement cancelled.")
                        return
                    if result["ok"]:
                        self.response_text.setPlainText(result["text"])
                        QMessageBox.information(self, "Success", "Text improved with AI!")
                    else:
                        QMessageBox.warning(self, "Error", f"AI improvement failed: {result['err']}")
                else:
        ...
        def prompt_for_gemini_api_key(self):
            ...
            desc_label = QLabel("Please enter your Google Gemini API key. It will be saved securely in your .env file.")
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                if api_key:
                    ...
                    if self.save_api_key_to_env(api_key):
                        ...
                        QMessageBox.warning(self, "Error", "Failed to save API key to .env file!")
                        return None
                else:
        ...
        def _get_stop_delay(self) -> float:
            ...
            try:
                ...
                logger.warning(f"Invalid APP_STOP_DELAY value '{delay_str}' ({type(delay_str).__name__}): {e}. Using default 1.5 seconds")
                return 1.5
        ...
        def stop_cursor(self):
            ...
            if self._is_cursor_control_disabled():
                ...
                QMessageBox.information(self, "Disabled", "Cursor control functionality is disabled in settings.")
                return
                ...
                try:
                    ...
                    if platform.system() == "Windows":
                        try:
                            import pythoncom
                            pythoncom.CoInitializeEx(pythoncom.COINIT_MULTITHREADED)
                            logger.debug("Stop worker thread: COM initialized as MULTITHREADED for pywinauto")
                        except Exception as e:
                            logger.debug(f"Stop worker thread: COM initialization note: {e}")
                    ...
                    if not _cached_cursor_windows:
                        logger.info("Discovering Cursor windows on-demand (first stop press)...")
                        _cached_cursor_windows = find_cursor_windows() or []
                        if _cached_cursor_windows:
                            logger.info(f"✓ Discovered {len(_cached_cursor_windows)} Cursor window(s)")
                        else:
                            logger.warning("No Cursor windows found")

                    if _cached_cursor_windows:
                        logger.info("Using cached Cursor windows...")
                        success = focus_and_send_stop_hotkey_to_any(_cached_cursor_windows, stop_delay)
                    else:
            ...
            if HAS_FOCUS_CURSOR:
                ...
                logger.warning("Cannot execute stop cursor - focus_cursor module not available")
...
def create_modern_feedback_dialog(agent_comment: str = "", project_dir: str = None) -> str:
    ...
    try:
        ...
        logger.info(f"Is main thread: {threading.current_thread() == threading.main_thread()}")
    except Exception as e:
    ...
    logger.debug(f"Platform: {platform.system()}, COM initialized: {com_init.COM_INITIALIZED if platform.system() == 'Windows' else 'N/A'}")
    ...
    try:
        ...
        logger.warning(f"Failed to set HiDPI attributes (may be normal from worker thread): {e}")
    ...
    try:
        ...
        logger.debug(f"Current thread before Qt call: {threading.current_thread().name}")
        app = QApplication.instance()
        logger.debug(f"QApplication.instance() returned: {app is not None}")
    except Exception as e:
        logger.error(f"CRITICAL: QApplication.instance() crashed from thread {threading.current_thread().name}: {e}", exc_info=True)
        raise RuntimeError(f"Qt threading error - cannot create UI from worker thread. Must run on main thread. Error: {e}") from e
    if app is None:
        ...
        try:
            ...
            def validate_app_dark_mode(value):
                ...
                if clean_value in valid_true_values:
                    ...
                    logger.warning(f"Invalid APP_DARK_MODE value '{value}'. Using default: true")
                    return "true"
            ...
            logger.info(f"Theme from environment - APP_DARK_MODE: '{app_dark_mode_raw}' -> validated: '{app_dark_mode_validated}' -> Dark mode: {preferred_dark}")
            ...
            for p in qss_paths:
                ...
                if p.exists():
                    logger.info(f"Loading QSS file in create_modern_feedback_dialog: {p}")
                    with open(p, 'r', encoding='utf-8') as f:
                        css_content = f.read()
                        app.setStyleSheet(css_content)
                        logger.info(f"Applied QSS to app in create_modern_feedback_dialog: {len(css_content)} characters")
                    break
                else:
                    logger.debug(f"QSS file not found in create_modern_feedback_dialog: {p}")
        except Exception as e:
    ...
    logger.info(f"Dialog closed - Result code: {dialog_result} (Accepted={QDialog.DialogCode.Accepted}, Rejected={QDialog.DialogCode.Rejected})")
...
def get_additional_instructions_config() -> dict:
    ...
    for instruction in custom_instructions:
        ...
        config[f"custom_{instruction_id}"] = _custom_instructions_state.get(instruction_id, False)
...
def load_custom_instructions() -> list:
    ...
    try:
        ...
        if not isinstance(instructions, list):
            logger.warning("Custom instructions 'instructions' is not a list; resetting")
            return []
        valid = []
...
def update_custom_instruction(instruction_id: str, titulo: str, prompt: str, persistent: bool) -> bool:
...
def toggle_cleanup_files(enabled: bool) -> str:
    ...
    return f"✅ Cleanup Files {status}. " + (
        "An instruction will be added to delete irrelevant files." if enabled
...
def toggle_wordpress_cs(enabled: bool) -> str:
    ...
    return f"✅ WordPress CS {status}. " + (
        "An instruction will be added about WordPress standards and sanitization." if enabled
...
def toggle_wp_dev(enabled: bool) -> str:
    ...
    return f"✅ WP Dev {status}. " + (
        "An instruction will be added about WordPress best practices." if enabled
...
def toggle_laravel_dev(enabled: bool) -> str:
    ...
    return f"✅ Laravel Dev {status}. " + (
        "An instruction will be added about Laravel best practices." if enabled
...
async def memory_save(
    ...
    try:
        ...
        index_data[project_hash] = {
            ...
            "event_counts": index_data.get(project_hash, {}).get("event_counts", {
                "milestone": 0,
                "bug_solved": 0,
                "user_preference": 0
            })
        ...
        try:
            ...
            if memory_file.exists():
                try:
                    ...
                    for entry in existing_entries_for_cache:
                        entry_text_hash = hashlib.md5(entry.get('description', '').encode('utf-8')).hexdigest()
            ...
            if cached_embedding:
                ...
                memory_entry["text_hash"] = text_hash
                logger.info(f"Generated new {EMBEDDING_PROVIDER} embedding for memory entry")

        except Exception as e:
            logger.warning(f"Failed to generate embedding: {e}")
            # Continue without embedding - fallback to text search
            memory_entry["text_hash"] = hashlib.md5(description.encode('utf-8')).hexdigest()
...
async def memory_call(
    ...
    await ctx.info(f"Retrieving memory from project: {project_dir} with query: '{query}'")

    try:
        # Get relevant memories using RAG
        relevant_memories = await get_relevant_memories(project_dir, query, event_type, limit, ctx)
        ...
        if memories:
            ...
            for i, entry in enumerate(memories, 1):
                ...
                event_type_display = entry.get('event_type', 'unknown').replace('_', ' ').title()
...
async def get_relevant_memories(
    ...
    try:
        ...
        all_entries = []
        event_types_to_search = ["milestone", "bug_solved", "user_preference"] if event_type == "all" else [event_type]
        ...
        if query.strip():
            ...
            relevant_entries = await perform_semantic_search(all_entries, query, limit, ctx)
        else:
        ...
        for entry in entries:
            if entry.get('embedding'):
                try:
                    ...
                    logger.warning(f"Failed to calculate similarity for entry {entry.get('id', 'unknown')}: {e}")
            else:
                ...
                if query_lower in description:
                    words_match = sum(1 for word in query_lower.split() if word in description)
                    total_words = len(query_lower.split())
                    text_score = words_match / total_words if total_words > 0 else 0

                    scored_entry = entry.copy()
                    scored_entry['relevance_score'] = text_score * 0.5  # Lower score for text matching
                    scored_entries.append(scored_entry)

        await ctx.info(f"Processed {entries_with_embeddings} entries with embeddings, {len(scored_entries)} total matches")
                    ...
                    with open(memory_file, 'r', encoding='utf-8') as f:
                        ...
                        if entries:
                            ...
                            for entry in recent_entries:
                                ...
                                title = entry.get('title', '')
                                description = entry.get('description', '')
                                formatted_entry = f"[{formatted_time}] {title}: {description}"
                ...
                if legacy_file.exists():
                    try:
                        with open(legacy_file, 'r', encoding='utf-8') as f:
                            ...
                            if content:
                                lines = content.split('\n')
                                memory_data["memories"][event_type] = lines[-5:] if len(lines) > 5 else lines
                            else:
                                memory_data["memories"][event_type] = []
                    except Exception:
                        memory_data["memories"][event_type] = []
...
async def ask_to_leader_project(
    ...
    try:
        ...
        auto_stop_enabled = os.getenv("APP_AUTO_STOP", "").strip().lower() in ("1", "true", "yes", "on")
        ...
        if auto_stop_enabled:
            ...
            try:
                if HAS_FOCUS_CURSOR:
                    ...
                    if success:
                        logger.info("Successfully focused cursor and sent stop hotkey in auto-stop mode")
                        await ctx.info("✅ Auto-stop: Successfully focused cursor and sent stop signal")
                        ...
                        await ctx.warning("⚠️ Auto-stop: Failed to focus cursor - continuing without stop signal")
                        return "⚠️ Auto-stop mode: Failed to focus cursor. Task completed without stop signal."
                else:
                    logger.warning("Focus cursor functionality not available in auto-stop mode")
                    await ctx.warning("⚠️ Auto-stop: Focus cursor functionality not available")
        ...
        try:
            ...
            logger.debug(f"Current thread: {threading.current_thread().name}")
            logger.debug(f"Parameters: agent_comment_length={len(agent_comment)}, project_dir={project_base_dir}")
            ...
            try:
                ...
                stdout, stderr = await result.communicate(input=agent_comment.encode('utf-8'))
                ...
                if "===UI_RESPONSE_START===" in stdout_text and "===UI_RESPONSE_END===" in stdout_text:
            ...
            logger.error(f"Unexpected exception launching UI subprocess: {e}", exc_info=True)
...
async def init_agent_files(
    ...
    try:
        ...
        await ctx.info(f"Setting up agent configuration files in: {target_directory}/{config_folder}")
        ...
        if result_parts:
            message = f"✅ Agent files initialized successfully in {config_folder}/\n" + "\n".join(result_parts)
        else:
...
def show_feedback_interface(agent_comment: str, ctx: Context = None, project_dir: str = None) -> str:
    ...
    logger.info(f"Agent comment length: {len(agent_comment)}")
    logger.info(f"Context provided: {ctx is not None} (should be None for thread safety)")
    logger.info(f"Project dir: {project_dir}")
    logger.info(f"Current thread: {threading.current_thread().name}")
    logger.info(f"Thread ID: {threading.get_ident()}")
    ...
    try:
        ...
        logger.debug(f"Calling create_modern_feedback_dialog with project_dir: {project_dir}")
        response = create_modern_feedback_dialog(agent_comment, project_dir)
        logger.debug(f"create_modern_feedback_dialog returned, response length: {len(response) if response else 0}")
